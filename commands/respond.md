...
Read `catalyst_state.json`. Find the active gate: `gate_1` or `gate_2` with status `active`, or — if both are passed and `final` isn't — the `final` gate. If nothing is active, explain that gates open when curveballs drop (or, post-Gate-2, that `/catalyst:review` runs the final board pre-read directly) and stop.
